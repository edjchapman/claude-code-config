...
- **Some invariants are declared, not derived.** `scripts/lib/primitives.py` declares which skills the cloud routines fire (`SCHEDULED_SKILLS`), which must stay user-only (`USER_ONLY_SKILLS`), and which `scripts/hooks/*.sh` are deliberately not hooks (`NON_HOOK_SCRIPTS` — every other tracked script there must be wired in `hooks/hooks.json`); `scripts/lib/architecture_catalogs.py` declares the platform's documented hook events (`DOCUMENTED_EVENTS` — a wired event must appear in it) and settings keys. `generate.py --check` fails on a violation. Adding a skill, hook script, or event in any of these categories means adding it there too.
- **Primitives are enumerated from git-tracked files**, never by directory scan (`config_common.tracked_files`). An untracked local-only skill or agent is invisible to the generator and the checkers alike — stage a new primitive before expecting it in a catalog.
- **No hand-written count of primitives.** `generate.py --check` fails on a sentence in `README.md` or `docs/architecture.md` (outside generated regions) that states one ("4 skills", "all three are opt-in"). Counts are generated; prose argues without numbers.
