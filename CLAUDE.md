...
- **Some invariants are declared, not derived.** `scripts/lib/primitives.py` declares the routine-fired skills (`SCHEDULED_SKILLS`), the user-only skills (`USER_ONLY_SKILLS`), and the `scripts/hooks/*.sh` that are deliberately not hooks (`NON_HOOK_SCRIPTS` — every other tracked one must be wired in `hooks/hooks.json`); `scripts/lib/architecture_catalogs.py` declares the platform's documented hook events (`DOCUMENTED_EVENTS` — a wired event must be in it) and settings keys. `generate.py --check` fails on a violation, and on a hand-written sentence in `README.md` or `docs/architecture.md` that states a count of primitives (counts are generated).
- **Primitives are git-tracked files.** Generator and checkers enumerate via `git ls-files`, so stage a new primitive before expecting it in a catalog; an untracked local-only one is invisible by design.
