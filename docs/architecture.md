...
- **`model`**: Default model alias (`opus` — broadly available, safe for plugin consumers). The maintainer's personal Fable pin (`claude-fable-5[1m]`) lives in `settings.personal.json.example`, merged by hand into `~/.claude/settings.local.json`: entitlement-gated pins must not ship in the universal file because `fallbackModel` does **not** cover entitlement — an unentitled model fails outright with a manual `/model` prompt (verified against the official settings + errors docs, 2026-07-22). History: `fable` → `opus` (Fable budget exhausted, 2026-07-14) → `claude-fable-5[1m]` in PR #79 → moved to the personal layer. Briefly removed in `cd66b43` as "deprecated", then restored after verifying the key is still canonical in the settings schema. Accepts aliases (`fable`, `opus`, `sonnet`, `haiku`), full model IDs, and 1M-context forms (e.g. `opus[1m]`).
