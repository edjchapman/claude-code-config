...
- **Wire-up — one file**: add the entry to `hooks/hooks.json` (under `hooks.<EventName>` inside the top-level `{"hooks": {...}}` wrapper); pre-commit regenerates `settings.json`'s hooks block from it (ADR-0001), or run `python3 scripts/generate.py`. Use a `command` value of `"${CLAUDE_PLUGIN_ROOT:-$(readlink ~/.claude/agents | xargs dirname)}/scripts/hooks/<name>.sh"` so it resolves in both plugin and symlink-global modes (the fallback anchors on the `agents` symlink — `settings.json` is a mirrored real file, not a symlink; ADR-0002).
