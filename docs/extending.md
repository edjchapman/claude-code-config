...
Examined and found _not_ colliding: `/refinement` (Jira ticket → analysis, the inverse of `to-tickets`, and user-only so it never auto-fires), `@test-engineer` (writes tests; `tdd` drives design), `/api-design` (REST shape; `codebase-design` is module seams), `/project-setup` (strictly broader than `setup-pre-commit`), and `wizard` against `/project-setup`, `/infrastructure`, `@devops-engineer` and `@database-architect` (those execute the work themselves; `wizard` emits a bash walkthrough for steps only a human can perform, such as a third-party dashboard, and its trigger says so — a different situation, not a competing method). ADRs were a three-way collision — `/adr`, `@documentation-writer`, and `domain-modeling` all write `docs/adr/` in three different formats — resolved by publishing the house format at the destination in [`docs/adr/README.md`](adr/README.md), which the vendored writer reads and this repo cannot edit into it.

### Bumping a vendored plugin

The pin is a git tag and the lockfile is the review surface. **The ref must be a branch or tag name, never a commit SHA** — Claude Code ≥ 2.1.266 installs a declared marketplace with `git clone --branch <ref>`, which git rejects for a SHA, so a SHA pin only ever loads on a machine where the clone already exists ([#152](https://github.com/edjchapman/claude-code-config/issues/152)); `generate.py --check` rejects one. The same version refuses a marketplace whose entry in `~/.claude/plugins/known_marketplaces.json` disagrees with the `extraKnownMarketplaces` declaration on any field, `ref` included, so the order below matters:

1. Set the new ref in `settings.json`'s `extraKnownMarketplaces`.
2. `python3 scripts/sync-global-settings.py` so `~/.claude/settings.json` declares it too.
3. `claude plugin marketplace update mattpocock`. If it reports "not found", the registry entry is stale: set its `ref` in `known_marketplaces.json` to the declared value by hand and retry.
4. `claude plugin update mattpocock-skills@mattpocock`, then confirm `claude plugin list` shows the expected version.
5. `python3 scripts/update-plugin-lock.py --ref <new ref>`. `generate.py --check` fails until settings and lockfile agree, in either direction.
6. **Read the lockfile diff.** Every changed `description` is a collision candidate against the table above; every added `model_invocable: true` entry is new always-loaded cost, and `check-context-budget.py` will say whether it fits.

The pin moved from an untagged `main` commit to `v1.2.3` in #152, which added `wizard` and `writing-for-agents` as model-invocable skills, absorbed by raising the context budget in the same change. `writing-for-agents` triggers on "creating or editing skills, or modifying AGENTS.md or CLAUDE.md" — the work this repository exists for — but it competes with a document (this guide), not with a dispatchable primitive, so it is additive rather than a collision; expect it to fire when editing primitives here, and defer to the Self-Extension Guide below where the two disagree.
...
There is **no third resolution for a vendored primitive**. `skillOverrides` (a `settings.json` key, values `on` / `name-only` / `user-invocable-only` / `off`) demotes a named skill's listing, but it is **inert for plugin-sourced skills** — verified against Claude Code 2.1.266 for [#159](https://github.com/edjchapman/claude-code-config/issues/159): the plugin loader tags every skill `source: "plugin"`, the runtime resolver returns `on` for that source before it reads the setting, and `/skills` shows the entry locked with source "plugin" rather than offering the override. So a third-party trigger collision is resolved by narrowing our own primitive or dropping the pin, and an over-guideline third-party description is either accepted at a recorded size (`ACCEPTED_OVERSIZE_ITEMS` in `scripts/check-context-budget.py`, which re-warns the moment upstream grows it) or the pin is dropped. Re-check the lock if a later CLI release changes it.
...
- **Wire-up — one file**: add the entry to `hooks/hooks.json` (under `hooks.<EventName>` inside the top-level `{"hooks": {...}}` wrapper); pre-commit regenerates `settings.json`'s hooks block from it (ADR-0001), or run `python3 scripts/generate.py`. Use a `command` value of `"${CLAUDE_PLUGIN_ROOT:-$(readlink ~/.claude/agents | xargs dirname)}/scripts/hooks/<name>.sh"` so it resolves in both plugin and global modes (the fallback anchors on the `agents` symlink — `settings.json` is a mirrored real file, not a symlink; ADR-0002).
