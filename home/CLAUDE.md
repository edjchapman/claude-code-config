...
- **Don't blanket-ignore `.claude/`.** A repo _commits_ `.claude/settings.json` + `.claude/hooks/` (the Claude-on-web bootstrap from `--tooling`); `.gitignore` only the personal bits — `.claude/{agents,commands,skills,rules}`, `settings.local.json`, `.mcp.json`.
